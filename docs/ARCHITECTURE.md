...
**PR 14b: right-click Edit/Remove Group.** PR 14b layers the user-visible interaction on top of PR 14a's foundation. Right-click on any pending redaction mark shows adaptive menu items: "Edit This Group... (N marks)" and "Remove This Group (N marks)" for multi-mark groups, or "Edit This Mark..." and "Remove This Mark" for singletons -- terminology follows the group size PageView resolves via ``list_redactions_grouped``. A new ``EditGroupDialog`` opens on Edit with prefilled fill/text values scoped to the group and a "Reset to Global" button enabled only when the group is customized. On OK, ``RedactionService.update_redaction_group`` is called with the group's normalized_text as key. On Reset, the current session defaults are applied instead. DocumentView's ``_on_remove_mark_requested`` from PR 12.5 is redefined here to be group-scoped: it now resolves the mark to its group and calls ``remove_redaction_group`` instead of the old single-mark ``remove_redaction`` -- semantics match the group-atomic principle established in PR 14a. Two new signals appear on PageView: ``edit_group_requested`` and ``remove_mark_requested`` (existing, redefined). Both carry ``(page_index, redaction_index)`` payloads; DocumentView's ``_resolve_group_for_mark`` helper turns these into a full RedactionGroup at slot execution.
...
**PR 16: Combine PDFs.** PR 16 delivers ``File -> Combine PDFs...``, a multi-source concatenation flow. The user picks source PDFs in a drag-reorderable list and combines them into a single new document that opens in a new tab. Order matters: for pending redaction groups whose extracted text matches across sources but whose styling differs, the last source wins on the collision (see M5 architecture Q&A for design rationale -- the group-atomic principle means all marks in a group must share styling, and last-source-wins matches the user's mental model where later sources override). Under the hood, combine uses PyMuPDF's ``insert_pdf`` for concatenation (annotations preserved by default), then applies reconciliation via a helper method ``_reconcile_combined_groups``. Reconciliation cannot happen before the first save -- PyMuPDF's annotation iterator does not surface annots from non-first ``insert_pdf`` calls until the doc is flattened via save. The flow is: concat sources into a target doc, save target to output_path, close target, reopen output_path, reconcile in-memory, and if any groups were reconciled, save to a temp path and rename over output_path (PyMuPDF disallows non-incremental save to the source path, so we use temp-and-rename for full-rewrite semantics without stale annotation data). Encrypted sources are rejected with a clear error -- combine requires decryption first. Session Global values are not affected by combine; users who want to un-reconcile a group after combine can use Reset in Manage Marks.

**PR 17a: Compare PDFs (text diff).** PR 17a delivers ``File -> Compare PDFs...``, a side-by-side text diff between two PDFs opened as a new tab in the app. The user picks a left and right document from either the currently-open tabs or external files via CompareDialog, and the result opens as a DiffView tab showing both docs with word-level highlights: deletions in light red on the left, insertions in light green on the right. Navigation via Previous / Next diff buttons scrolls both panes to the current change and paints a bold blue outline around it (adjacent same-line words merged into one outline box; wrapping across lines produces separate boxes per line). Sync scroll (default on) keeps both panes aligned vertically and horizontally. The diff algorithm is standard sequence matching (Python's ``difflib.SequenceMatcher``) applied to word sequences extracted via a new adapter primitive ``extract_words_with_boxes``. This is a text-only diff: moved sections appear as both a deletion and an insertion, formatting-only changes are invisible, and image differences are not detected. Users are warned about the text-only limitation via a footer label in DiffView. Layout-aware comparison (move detection, page reorder recognition) and image / visual diff are deferred: image / full-page visual diff to PR 17b; layout-aware comparison to a later PR. MainWindow tab machinery was updated to accept DiffView tabs alongside DocumentView tabs -- close handling, tab-changed status bar updates (page/zoom/tool indicators hidden for DiffView), and window title all now branch on tab type.

**PR 17b: Compare PDFs (image diff).** PR 17b extends the PR 17a text diff with image-level comparison. The same ``File -> Compare PDFs...`` flow now also detects when embedded images have been added, removed, or replaced between the two documents. Image changes are surfaced through a new highlight layer (magenta outline, distinct from red/green text highlights) and are included in the Previous / Next diff navigation cycle alongside text diffs. Position label distinguishes them: ``(image added)``, ``(image removed)``, ``(image replaced)``. Matching is position-based: for each page, images in doc A are paired with images in doc B by bbox center distance (tolerance 20 pt). Matched pairs with differing MD5 hashes are ``replaced``; unmatched A images are ``removed``; unmatched B images are ``added``. Cross-page matching is not attempted -- an image moved from page 3 to page 5 will show as removed on page 3 + added on page 5. Perceptual hashing (pHash) is not used; byte-identical comparison flags re-compressed images as different (which is technically correct), and the position-based algorithm has no perceptual tolerance. Header stats gain a ``, N image change(s)`` suffix when any exist. New adapter primitive ``extract_images_with_bboxes`` returns per-occurrence ``(page_index, bbox, raw_bytes)`` triples using PyMuPDF's ``page.get_images(full=True)`` + ``page.get_image_bbox`` + ``doc.extract_image``. Full-page pixel diff was considered and explicitly dropped from the roadmap: on text-only pages it duplicates PR 17a's content diff with a lower-level UX; it delivers genuine new value only for docs where formatting-only or layout-only differences matter, which is a narrow enough use case that the side-by-side rendering the DiffView already provides is sufficient. Milestone 6 closes with PR 17b.

**PR 17.6: Redaction in continuous view mode.** PR 17.6 removes a design-level limitation from earlier redaction PRs (12 / 12.5 / 14b) that gated drag-to-mark and right-click Edit/Remove Group on single-page view only. In continuous view all pages are stacked vertically in one scene with ``_PAGE_SPACING`` between them, so the drag path needs to (a) identify which page the drag ANCHORED on -- that's the page the mark belongs to, regardless of where the drag ends -- (b) subtract the page's scene y-offset to get page-local coordinates, and (c) clamp the drag END to the starting page's scene range so drags spanning multiple pages produce one mark on the starting page rather than fragments across two. A small ``_page_at_scene_y(y)`` helper walks ``_page_offsets`` to find the page whose vertical range contains y. Both ``_commit_redaction_drag`` and ``_hit_test_redaction`` now branch on view mode and use the helper in continuous mode; single-page mode still uses the simpler ``_current_page`` path unchanged. Design decisions locked in scoping: clamp drags to the anchor page (rather than splitting into per-page marks or rejecting), no visual pre-render indicator of which page owns the drag (users already see the temp rectangle on the scene during drag).
...
- PR 17a (text diff, **shipped**): word-level side-by-side comparison with navigation. PR 17b (image diff, **shipped**): position-based image matching with add/remove/replace detection. **Milestone 6 complete.**
