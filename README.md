...
- Compare two PDFs: File -> Compare PDFs... opens a side-by-side text-only comparison of two documents with word-level highlights (deletions on the left, insertions on the right). Previous/Next diff buttons navigate between changes. This is a text-only comparison: moved sections appear as both deletion and insertion, and formatting-only or image-only differences are not detected.
...
- Image/visual diff between two PDFs (text diff shipped in PR 17a)
